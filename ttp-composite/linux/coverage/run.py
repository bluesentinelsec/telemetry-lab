...
def score(case, execution, alerts, collection_ok, selected):
    behavior_ok = execution.returncode == 0 and ('CASE_OK ' + case['id']) in execution.stdout.splitlines()
    matches = sorted({a['rule'] for a in alerts})
    target = case.get('rule')
...
def execute(case, config, output, image, functional_only=False):
    ...
    exe = '/opt/coverage/' + config + '/falco_cases'
    ...
    try:
        ...
        command(['docker', 'exec', cid, exe, '--prepare'])
        ...
        execution = subprocess.run(['docker', 'exec', cid, exe, case['id']],
...
def main():
    ...
    provenance['binary_sha256']=command(['docker','run','--rm','--network','none',image_id,
        'sha256sum','/opt/coverage/linux-c-glibc/falco_cases',
        '/opt/coverage/linux-c-musl/falco_cases','/opt/coverage/helper'])
    ...
    for index,item in enumerate(plan):
        ...
        try:
            ...
            record={'case':item['case']['id'],'config':item['config'],'valid':False,
                    'behavior_ok':False,'error':str(error)}
    ...
    summary={'executions':len(records),'valid':sum(r['valid'] for r in records),
             ...
             'negative_controls_pass':all(r.get('valid') and r.get('negative_control_ok') for r in records if r['case']=='negative'),
    ...
    return 0 if all(r['valid'] for r in records) and summary['negative_controls_pass'] and len(qualified)==len(cases)-1 else 1
