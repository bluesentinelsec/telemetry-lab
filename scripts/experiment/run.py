...
def main():
    ...
    p.add_argument('--cohort',choices=('primitives','composites'),required=True)
    ...
    a=p.parse_args()
    if a.repetitions<1 or a.max_retries<0 or not (0<=a.retry_delay<=3600):p.error('Invalid repetition/retry limits')
    ...
    with host_lock():
        ...
        elif os_name=='linux':adapter=LinuxComposites(coverage,a.image,verify)
        else:adapter=WindowsComposites(a.bundle,coverage,verify)
        inventory=None
        if a.inventory:
        ...
        summary=campaign(a.output,plan,adapter,0 if a.no_retry else a.max_retries,a.retry_delay,
                         provenance=provenance,inventory=inventory)
